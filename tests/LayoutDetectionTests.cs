...
/// <summary>Tests for <see cref="CurveLayout.TryDetect"/>, which recovers the status curve's record layout
/// (stride and absolute volt/freq column offsets) from a raw buffer by finding the ascending-voltage array.
/// Exercised against synthetic buffers, so known layouts are covered without a GPU.</summary>
public class LayoutDetectionTests
{
    ...
    [Fact]
    public void WholesaleCollapse_StillFindsTheVoltageAxis_ButReportsFreqUndetected()
    {
        // Every clock collapsed below a boost clock (the transitional shape): the voltage column is
        // still valid, so stride and the voltage column are found, but the freq column can't be identified.
        byte[] buf = BuildStatus(baseOffset: 0x40, stride: 28, voltOffset: 0x0C, freqOffset: 0x08,
    ...
    [Fact]
    public void MatchesCompiled_TrueWhenColumnsMatchTheBuild()
    {
        ...
        Assert.True(match.MatchesCompiled());
    }

    [Fact]
    public void MatchesCompiled_FalseWhenAColumnDiffers()
    {
        var off = new CurveLayout(NvApi.StatusEntryStride,
            ...
            NvApi.StatusEntryBase + NvApi.StatusFreqOffset, 127);
        Assert.False(off.MatchesCompiled());
    }

    [Fact]
    public void MatchesCompiled_SkipsTheFreqColumnWhenUndetected()
    {
        // Stride and voltage match; the freq column is undetected (-1, a collapsed read), so it isn't
        // held against the build - the read path still counts as fitting.
        var undetected = new CurveLayout(NvApi.StatusEntryStride,
            NvApi.StatusEntryBase + NvApi.StatusVoltOffset, -1, 127);
        Assert.True(undetected.MatchesCompiled());
    }
