...
///
/// Detecting the layout and comparing it to the offsets compiled into <see cref="NvApi"/> is autonomous
/// confirmation that the hardware-specific offsets fit this GPU — the same check the <c>layout</c>
/// diagnostic prints, run inline by the read/write paths so they can trust (or refuse) the offsets.
/// </summary>
internal readonly record struct CurveLayout(int Stride, int VoltColumn, int FreqColumn, int Count)
{
    ...
    /// <summary>Describes how this detected layout differs from the offsets compiled into <see cref="NvApi"/>,
    /// or null when it matches (the frequency column is ignored when idle and undetected). A null result is
    /// autonomous confirmation that the compiled offsets fit this GPU.</summary>
    public string? MismatchVsCompiled()
    {
        ...
        string freq = FreqColumn >= 0 ? $" freq +0x{FreqColumn - b:X2}" : string.Empty;
        return $"detected stride {Stride} volt +0x{VoltColumn - b:X2}{freq}; "
               + $"compiled stride {NvApi.StatusEntryStride} volt +0x{NvApi.StatusVoltOffset:X2} freq +0x{NvApi.StatusFreqOffset:X2}";
    }
